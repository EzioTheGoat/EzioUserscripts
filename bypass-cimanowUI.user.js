...
(function () {
  ...
  const ordinalRegex = new RegExp(`(?:${ordinalWords})`, "gi");
  ...
  function extractMediaInfo() {
    ...
    if (sm) {
      info.season = parseInt(sm[1]);
    } else {
      const smOrd = cleaned.match(
        new RegExp(`(?:ج|جزء|موسم|الموسم|season|s)[\\s\\-_]*(${ordinalWords})`, "i")
      );
      if (smOrd) {
        const key = smOrd[1].trim();
        const matchedKey = Object.keys(arabicOrdinals).find(
          (k) => k === key
        );
        if (matchedKey) info.season = arabicOrdinals[matchedKey];
      } else { }
    }
    ...
    if (info.season === null) {
      const standaloneOrd = cleaned.match(
        new RegExp(`(?:^|[\\s\\-_])(${ordinalWords})(?:[\\s\\-_]|$)`, "i")
      );
      if (standaloneOrd) {
        const key = standaloneOrd[1].trim();
        const matchedKey = Object.keys(arabicOrdinals).find((k) => k === key);
        if (matchedKey) {
          info.season = arabicOrdinals[matchedKey];
        }
      }
    }
    ...
    let slug = cleaned
      ...
      .replace(
        new RegExp(`[\\s\\-_]*(ج|جزء|موسم|الموسم|season|s)[\\s\\-_]*(${ordinalWords})`, "gi"),
        ""
      )
      .replace(
